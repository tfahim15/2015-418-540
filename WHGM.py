    ...
    for h in hypergraphs:
        ...
        for key in temp_extensions:
            ...
            if key in extensions:
                weight, ubw = extensions[key]
                extensions[key] = (w + weight, ubw + h.get_weight())
            else:
                extensions[key] = (w, h.get_weight())
    return extensions
...
domain = domains[3]
hypergraphs = load_hypergraphs_from_file(open("weighted_hypergraph_data/" + domain + ".txt").readlines())
out = open("patterns/weighted/" + domain + ".txt", "w")
...
# WHGM algorithm
def WHGM(code, hypergraphs, min_w):
    ...
    for c in codes:
        ...
        if is_minimum(c) is False:  # Remove this check for getting naive algorithm
            continue
        if ubw < min_w:
            print("not ubw", c.get_extensions(), weight,ubw)
            continue
        if weight >= min_w:
            out.write("t # " + str(found) + "\n")
            out.write(str(c.get_hypergraph())+"\n")
            found += 1
            print("found", c.get_extensions(), weight,ubw)
        else:
            print("extend", c.get_extensions(), weight, ubw)
        try:
            WHGM(c, hypergraphs, min_w)
        except MemoryError:
            err += 1
            pass
...
print(min_w)
t = time.time()
WHGM(CandidateCode(), hypergraphs, min_w)
